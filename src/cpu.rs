...
impl Cpu {
    ...
    fn call(&mut self) -> u32 {
        ...
        match opcode {
            0x00 => { 1 },
            0x01 => { let v = self.fetchword(); self.setbc(v); 3 },
            0x02 => { self.mmu.wb(self.bc(), self.a); 2 },
            0x03 => { self.setbc(self.bc().wrapping_add(1)); 2 },
            0x04 => { self.b = self.alu_inc(self.b); 1 },
            0x05 => { self.b = self.alu_dec(self.b); 1 },
            0x06 => { self.b = self.fetchbyte(); 2 },
            0x07 => { self.a = self.alu_rlc(self.a); self.flag(Z, false); 1 },
            0x08 => { let a = self.fetchword(); self.mmu.ww(a, self.sp); 5 },
            0x09 => { self.alu_add16(self.bc()); 2 },
            0x0A => { self.a = self.mmu.rb(self.bc()); 2 },
            0x0B => { self.setbc(self.bc().wrapping_sub(1)); 2 },
            0x0C => { self.c = self.alu_inc(self.c); 1 },
            0x0D => { self.c = self.alu_dec(self.c); 1 },
            0x0E => { self.c = self.fetchbyte(); 2 },
            0x0F => { self.a = self.alu_rrc(self.a); self.flag(Z, false); 1 },
            0x10 => { self.mmu.switch_speed(); 1 }, // STOP
            0x11 => { let v = self.fetchword(); self.setde(v); 3 },
            0x12 => { self.mmu.wb(self.de(), self.a); 2 },
            0x13 => { self.setde(self.de().wrapping_add(1)); 2 },
            0x14 => { self.d = self.alu_inc(self.d); 1 },
            0x15 => { self.d = self.alu_dec(self.d); 1 },
            0x16 => { self.d = self.fetchbyte(); 2 },
            0x17 => { self.a = self.alu_rl(self.a); self.flag(Z, false); 1 },
            0x18 => { self.cpu_jr(); 3 },
            0x19 => { self.alu_add16(self.de()); 2 },
            0x1A => { self.a = self.mmu.rb(self.de()); 2 },
            0x1B => { self.setde(self.de().wrapping_sub(1)); 2 },
            0x1C => { self.e = self.alu_inc(self.e); 1 },
            0x1D => { self.e = self.alu_dec(self.e); 1 },
            0x1E => { self.e = self.fetchbyte(); 2 },
            0x1F => { self.a = self.alu_rr(self.a); self.flag(Z, false); 1 },
            0x20 => { if !self.getflag(Z) { self.cpu_jr(); 3 } else { self.pc += 1; 2 } },
            0x21 => { let v = self.fetchword(); self.sethl(v); 3 },
            0x22 => { let addr = self.hli(); self.mmu.wb(addr, self.a); 2 },
            0x23 => { let v = self.hl().wrapping_add(1); self.sethl(v); 2 },
            0x24 => { self.h = self.alu_inc(self.h); 1 },
            0x25 => { self.h = self.alu_dec(self.h); 1 },
            0x26 => { self.h = self.fetchbyte(); 2 },
            0x27 => { self.alu_daa(); 1 },
            0x28 => { if self.getflag(Z) { self.cpu_jr(); 3 } else { self.pc += 1; 2  } },
            0x29 => { let v = self.hl(); self.alu_add16(v); 2 },
            0x2A => { let addr = self.hli(); self.a = self.mmu.rb(addr); 2 },
            0x2B => { let v = self.hl().wrapping_sub(1); self.sethl(v); 2 },
            0x2C => { self.l = self.alu_inc(self.l); 1 },
            0x2D => { self.l = self.alu_dec(self.l); 1 },
            0x2E => { self.l = self.fetchbyte(); 2 },
            0x2F => { self.a = !self.a; self.flag(H, true); self.flag(N, true); 1 },
            0x30 => { if !self.getflag(C) { self.cpu_jr(); 3 } else { self.pc += 1; 2 } },
            0x31 => { self.sp = self.fetchword(); 3 },
            ...
            0x36 => { let v = self.fetchbyte(); self.mmu.wb(self.hl(), v); 3 },
            0x37 => { self.flag(C, true); self.flag(H, false); self.flag(N, false); 1 },
            0x38 => { if self.getflag(C) { self.cpu_jr(); 3 } else { self.pc += 1; 2  } },
            ...
            0x3E => { self.a = self.fetchbyte(); 2 },
            0x3F => { let v = !self.getflag(C); self.flag(C, v); self.flag(H, false); self.flag(N, false); 1 },
            0x40 => { 1 },
            ...
            0xC0 => { if !self.getflag(Z) { self.pc = self.popstack(); 5 } else { 2 } },
            0xC1 => { let v = self.popstack(); self.setbc(v); 3 },
            0xC2 => { if !self.getflag(Z) { self.pc = self.fetchword(); 4 } else { self.pc += 2; 3 } },
            0xC3 => { self.pc = self.fetchword(); 4 },
            0xC4 => { if !self.getflag(Z) { self.pushstack(self.pc + 2); self.pc = self.fetchword(); 6 } else { self.pc += 2; 3 } },
            0xC5 => { self.pushstack(self.bc()); 4 },
            0xC6 => { let v = self.fetchbyte(); self.alu_add(v, false); 2 },
            0xC7 => { self.pushstack(self.pc); self.pc = 0x00; 4 },
            0xC8 => { if self.getflag(Z) { self.pc = self.popstack(); 5 } else { 2 } },
            0xC9 => { self.pc = self.popstack(); 4 },
            0xCA => { if self.getflag(Z) { self.pc = self.fetchword(); 4 } else { self.pc += 2; 3 } },
            0xCB => { self.call_cb() },
            0xCC => { if self.getflag(Z) { self.pushstack(self.pc + 2); self.pc = self.fetchword(); 6 } else { self.pc += 2; 3 } },
            0xCD => { self.pushstack(self.pc + 2); self.pc = self.fetchword(); 6 },
            0xCE => { let v = self.fetchbyte(); self.alu_add(v, true); 2 },
            0xCF => { self.pushstack(self.pc); self.pc = 0x08; 4 },
            0xD0 => { if !self.getflag(C) { self.pc = self.popstack(); 5 } else { 2 } },
            0xD1 => { let v = self.popstack(); self.setde(v); 3 },
            0xD2 => { if !self.getflag(C) { self.pc = self.fetchword(); 4 } else { self.pc += 2; 3 } },
            0xD4 => { if !self.getflag(C) { self.pushstack(self.pc + 2); self.pc = self.fetchword(); 6 } else { self.pc += 2; 3 } },
            0xD5 => { self.pushstack(self.de()); 4 },
            0xD6 => { let v = self.fetchbyte(); self.alu_sub(v, false); 2 },
            0xD7 => { self.pushstack(self.pc); self.pc = 0x10; 4 },
            0xD8 => { if self.getflag(C) { self.pc = self.popstack(); 5 } else { 2 } },
            0xD9 => { self.pc = self.popstack(); self.setei = 1; 4 },
            0xDA => { if self.getflag(C) { self.pc = self.fetchword(); 4 } else { self.pc += 2; 3 } },
            0xDC => { if self.getflag(C) { self.pushstack(self.pc + 2); self.pc = self.fetchword(); 6 } else { self.pc += 2; 3 } },
            0xDE => { let v = self.fetchbyte(); self.alu_sub(v, true); 2 },
            0xDF => { self.pushstack(self.pc); self.pc = 0x18; 4 },
            0xE0 => { let a = 0xFF00 | self.fetchbyte() as u16; self.mmu.wb(a, self.a); 3 },
            0xE1 => { let v = self.popstack(); self.sethl(v); 3 },
            0xE2 => { self.mmu.wb(0xFF00 | self.c as u16, self.a); 2 },
            0xE5 => { self.pushstack(self.hl()); 4 },
            0xE6 => { let v = self.fetchbyte(); self.alu_and(v); 2 },
            0xE7 => { self.pushstack(self.pc); self.pc = 0x20; 4 },
            0xE8 => { self.sp = self.alu_add16imm(self.sp); 4 },
            0xE9 => { self.pc = self.hl(); 1 },
            0xEA => { let a = self.fetchword(); self.mmu.wb(a, self.a); 4 },
            0xEE => { let v = self.fetchbyte(); self.alu_xor(v); 2 },
            0xEF => { self.pushstack(self.pc); self.pc = 0x28; 4 },
            0xF0 => { let a = 0xFF00 | self.fetchbyte() as u16; self.a = self.mmu.rb(a); 3 },
            0xF1 => { let v = self.popstack() & 0xFFF0; self.setaf(v); 3 },
            0xF2 => { self.a = self.mmu.rb(0xFF00 | self.c as u16); 2 },
            0xF3 => { self.setdi = 2; 1 },
            0xF5 => { self.pushstack(self.af()); 4 },
            0xF6 => { let v = self.fetchbyte(); self.alu_or(v); 2 },
            0xF7 => { self.pushstack(self.pc); self.pc = 0x30; 4 },
            0xF8 => { let r = self.alu_add16imm(self.sp); self.sethl(r); 3 },
            0xF9 => { self.sp = self.hl(); 2 },
            0xFA => { let a = self.fetchword(); self.a = self.mmu.rb(a); 4 },
            0xFB => { self.setei = 2; 1 },
    ...
    fn alu_add(&mut self, b: u8, usec: bool) {
        ...
        self.flag(N, false);
        self.flag(C, (a as u16) + (b as u16) + (c as u16) > 0xFF);
        self.a = r;
    }
    ...
    fn alu_add16(&mut self, b: u16) {
        ...
        self.flag(H, (a & 0x07FF) + (b & 0x07FF) > 0x07FF);
        self.flag(N, false);
