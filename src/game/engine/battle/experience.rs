...
fn divide_exp_data_by_num_mons_gaining_exp(cpu: &mut Cpu) {
    ...
    cpu.a = cpu.read_byte(wram::W_PARTY_GAIN_EXP_FLAGS);
    cpu.pc += 3;
    cpu.cycle(16);
    ...
    divide_exp_data_by_num_mons_gaining_exp_count_set_bits_loop(cpu);
}

// loop to count set bits in wPartyGainExpFlags
fn divide_exp_data_by_num_mons_gaining_exp_count_set_bits_loop(cpu: &mut Cpu) {
    cpu.pc = 0x5484;
    ...
    {
        let carry = if cpu.flag(CpuFlag::C) { 1 } else { 0 };
        cpu.set_flag(CpuFlag::H, (cpu.a & 0x0f) + (cpu.d & 0x0f) + carry > 0x0f);
        cpu.set_flag(
            CpuFlag::C,
            (cpu.a as u16) + (cpu.d as u16) + (carry as u16) > 0xff,
        );
        cpu.a = cpu.a.wrapping_add(cpu.d).wrapping_add(carry);
    }
    cpu.set_flag(CpuFlag::Z, cpu.a == 0);
    cpu.set_flag(CpuFlag::N, false);
    cpu.pc += 1;
    ...
    if !cpu.flag(CpuFlag::Z) {
        cpu.cycle(12);
        return divide_exp_data_by_num_mons_gaining_exp_count_set_bits_loop(cpu);
    } else {
        cpu.pc += 2;
        cpu.cycle(8);
    }
    ...
    // store number of mons gaining exp
    // ld [wTempByteValue], a
    cpu.write_byte(wram::W_TEMP_BYTE_VALUE, cpu.a);
    cpu.pc += 3;
    cpu.cycle(16);
    ...
    cpu.c = (wram::W_ENEMY_MON_BASE_EXP + 1 - wram::W_ENEMY_MON_BASE_STATS) as u8;
    cpu.pc += 1;
    cpu.cycle(4);

    divide_exp_data_by_num_mons_gaining_exp_divide_loop(cpu);
}

fn divide_exp_data_by_num_mons_gaining_exp_divide_loop(cpu: &mut Cpu) {
    cpu.pc = 0x5497;
    ...
    // ldh [hDividend], a
    cpu.write_byte(hram::H_DIVIDEND, cpu.a);
    cpu.pc += 2;
    cpu.cycle(12);
    ...
    // ldh [hDividend + 1], a
    cpu.write_byte(hram::H_DIVIDEND + 1, cpu.a);
    cpu.pc += 2;
    cpu.cycle(12);

    // ld a, [wTempByteValue]
    cpu.a = cpu.read_byte(wram::W_TEMP_BYTE_VALUE);
    cpu.pc += 3;
    cpu.cycle(16);
    ...
    // divide value by number of mons gaining exp
    // call Divide
    {
        cpu.pc += 3;
        let pc = cpu.pc;
    ...
    if !cpu.flag(CpuFlag::Z) {
        cpu.cycle(12);
        return divide_exp_data_by_num_mons_gaining_exp_divide_loop(cpu);
    } else {
        cpu.pc += 2;
        cpu.cycle(8);
    }
